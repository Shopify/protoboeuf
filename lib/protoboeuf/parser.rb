...
module ProtoBoeuf
  ...
  class << self
    ...
    def check_enum_collision(enums)
      names = Set.new
      enums.each do |enum|
    ...
    def parse_field_type(input)
      ...
      type_name = ident
      type_name << "." << input.read_ident while input.match(".")

      type_name
    end

    # Parse a package name, e.g. .foo.bar.bif
    def parse_package_name(input)
      name = +""
      ...
      name << "." << input.read_ident while input.match(".")

      name
    end

    # Parse a composite name, e.g. foo.bar.bif
    def parse_composite_name(input)
      name = +""
      name << input.read_ident
      name << "." << input.read_ident while input.match(".")
      name
    ...
    def parse_field_options(input)
      ...
      loop do
        ...
        if input.match("(")
          opt_name = parse_composite_name(input) # rubocop:disable Style/IdenticalConditionalBranches
          input.expect(")")
        else
          opt_name = parse_composite_name(input) # rubocop:disable Style/IdenticalConditionalBranches
        end
    ...
    def parse_body(input, pos, inside_message, top_enums, top_messages, name_stack, oneof_decls, is_proto3) # rubocop:disable Metrics/ParameterLists
      fields = []
      messages = []
      enums = []
      reserved = []
      ...
      until input.match("}")
        ...
        if inside_message
          ...
          if input.match("oneof")
            ...
            oneof.fields.each do |field|
              if field.type_name.count(".") == 1 # rubocop:disable Metrics/BlockNesting
        ...
        if type.is_a?(MapType)
          ...
          nested_msg = Message.new(
            derived_message_name,
          ...
          fields << map_field
        elsif qualifier == :optional && is_proto3
          fields << Field.new(
            label(qualifier),
            qualify(top_messages + top_enums, nested_type + enums, name_stack, type),
            get_type(type, top_enums + enums),
            name,
            number,
            options,
      ...
      [fields, messages, enums, nested_type]
    end
    ...
    def qualify(top_messages, sibling_messages, stack, name)
      case name
      ...
      when "uint32", "bool", "double", "float", "int64", "uint64", "int32",
        ...
        if name =~ /\./ # if the name has dots, we'll assume it's fully qualified
          "." + name
        elsif sibling_messages.find { |msg| msg.name == name }
          # If it's in a sibling, then use our current context
          "." + (stack + [name]).join(".")
        elsif !top_messages.find { |msg| msg.name == name }
    ...
    def parse_message(input, pos, enums, top_messages, name_stack, is_proto3) # rubocop:disable Metrics/ParameterLists
      input.eat_ws
      message_name = input.read_ident
      oneof_decls = []
      fields, messages, enums, nested_type = parse_body(
        input,
        pos,
        true,
        enums,
        top_messages,
        name_stack + [message_name],
        oneof_decls,
        is_proto3,
      )
    ...
    def parse_oneof(input, pos, enums, top_messages, name_stack, oneof_decls, is_proto3) # rubocop:disable Metrics/ParameterLists
      input.eat_ws
    ...
    def parse_enum(input, pos)
      ...
      constants.each do |constant|
        # For each reserved range
        reserved.each do |r|
          next unless (r.respond_to?(:include?) && r.include?(constant.number)) || r == constant.number

          raise Parser::Error.new(
            "constant #{constant.name} uses reserved constant number #{constant.number}",
            constant.pos,
          )
        end
      end
  ...
  class Input
    ...
    def start_with?(str)
      @src[@cur_idx...(@cur_idx + str.size)] == str
    end
